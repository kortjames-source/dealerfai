...
<body>
  ...
  <header>
    ...
    <div class="logout">
      <?php if ($isAdmin): ?>
        <a href="admin_error_alerts.php">Alerts<?php if ($adminAlertCount > 0): ?> <span
  ...
  <main>
    <form method="post" id="create-deal-form">
      ...
      <div class="card">
        ...
        <input type="text" name="deal_number"
          value="<?= htmlspecialchars($_SESSION['deal_draft']['deal_number'] ?? '') ?>" required>

        <label>Customer Number</label>
        <input type="text" name="customer_number"
          value="<?= htmlspecialchars($_SESSION['deal_draft']['customer_number'] ?? '') ?>">

        <label>Customer Name</label>
        <input type="text" id="customer_name" name="customer_name"
          value="<?= htmlspecialchars($_SESSION['deal_draft']['customer_name'] ?? '') ?>" required>

        <label>Phone Number</label>
        <input type="text" name="customer_phone"
          value="<?= htmlspecialchars($_SESSION['deal_draft']['customer_phone'] ?? '') ?>">

        <label>Email</label>
        <input type="email" name="customer_email"
          value="<?= htmlspecialchars($_SESSION['deal_draft']['customer_email'] ?? '') ?>">
        ...
        <select name="customer_type" id="customer_type" onchange="toggleCustomerTypeUi()" required>
          ...
          <option value="professional" <?= $selectedCustomerType === 'professional' ? 'selected' : '' ?>>Professional
            (Business Buyer)</option>
          <option value="commercial" <?= $selectedCustomerType === 'commercial' ? 'selected' : '' ?>>Commercial (Business)
          </option>
        </select>

        <div id="business_block"
          style="display:none; margin-top: 10px; padding: 12px; border: 1px solid #e5e7eb; border-radius: 8px; background: #f8fafc;">
          ...
          <input type="text" name="business_name" id="business_name"
            value="<?= htmlspecialchars($_SESSION['deal_draft']['business_name'] ?? '') ?>">

          <div style="font-size: 12px; color: #475467; margin-top: 6px;">
            For Professional/Commercial deals we can collect more business details from the customer during the
            application.
          </div>
        </div>

        <label>
          <input type="checkbox" id="co_app_required" name="co_app_required" value="1"
            <?= !empty($_SESSION['deal_draft']['co_app_required']) ? 'checked' : '' ?> onchange="toggleCoSignerUi()">
          Add Co-Signer
        </label>
        <div id="co_app_name_wrap" class="d-none">
          ...
          <input type="text" id="co_app_name" name="co_app_name"
            value="<?= htmlspecialchars($_SESSION['deal_draft']['co_app_name'] ?? '') ?>">
        </div>
