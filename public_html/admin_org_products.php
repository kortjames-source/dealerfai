...
$orgs = $db->query("SELECT id, name, org_kind, logic_type FROM organizations WHERE org_kind = 'group' ORDER BY name ASC")
    ->fetchAll(PDO::FETCH_ASSOC);

$selectedOrgId = (int)($_POST['org_id'] ?? ($_GET['org_id'] ?? 0));
if ($selectedOrgId === 0 && !empty($orgs)) {
...
if (!$selectedOrg && !empty($orgs)) {
    $selectedOrg = $orgs[0];
    $selectedOrgId = (int)$selectedOrg['id'];
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save_org_products') {
    ...
    if ($selectedOrgId <= 0) {
        ...
        foreach ($variants as $variant) {
            ...
            $stmt->execute([$productId, $code, $provider, (string)$selectedOrgId, $isEnabled]);
        }
        $message = 'Organization availability saved.';
    }
}
$csrfToken = dealerfai_csrf_get_token();

$logicType = $selectedOrg ? ($selectedOrg['logic_type'] ?? '') : '';
$products = [];
if ($selectedOrgId > 0) {
    $stmt = $db->prepare("
        SELECT p.code,
...
<head>
  ...
  <meta charset="UTF-8">
  <title>Admin - Organization Product Availability</title>
...
<body>
  ...
  <div class="container">
    <h2>Organization Product Availability</h2>
    <p class="muted">Select an organization and choose which products are enabled for all stores in the group.</p>
    ...
    <form method="post" class="mt-16">
      ...
      <input type="hidden" name="action" value="save_org_products">
      ...
      <table id="productsTable">
        <thead>
          <tr>
            ...
            <th style="width:30%; cursor:pointer;" data-sort="product">Product</th>
          ...
          <?php foreach ($products as $product): ?>
            <?php
              ...
              if ($orgOverrideVariant !== null) {
                  $statusLabel = (int)$orgOverrideVariant === 1 ? 'Enabled for org (variant)' : 'Disabled for org (variant)';
                  $statusClass = 'tag store';
              } elseif ($orgOverrideProvider !== null) {
                  $statusLabel = (int)$orgOverrideProvider === 1 ? 'Enabled for org (provider)' : 'Disabled for org (provider)';
                  $statusClass = 'tag store';
              } elseif ($orgOverrideAny !== null) {
                  $statusLabel = (int)$orgOverrideAny === 1 ? 'Enabled for org (all providers)' : 'Disabled for org (all providers)';
                  $statusClass = 'tag store';
              } else {
